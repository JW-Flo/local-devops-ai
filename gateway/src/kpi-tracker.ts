...
export type CycleSnapshot = {
  ...
  reposUpdated: number;
  // PR Quality
  validationPassRate: number;     // 0-1, fraction that passed validation gate
  prsCreated: number;
  prsMerged: number;
  reviewsApproved: number;
  reviewsChangesRequested: number;
  reviewFixesApplied: number;
};
...
export type KPIDashboard = {
  ...
  prQuality: {
    ...
    totalReviewFixesApplied: number;
  };
  // Top failure reasons
    ...
    await withDb((db) => {
      db.exec(`
        CREATE TABLE IF NOT EXISTS kpi_cycles (
          ...
          review_fixes_applied INTEGER DEFAULT 0
        )
      `);
      db.exec(`
    ...
    await withDb((db) => {
      const stmt = db.prepare(`
        INSERT INTO kpi_cycles (
          ...
          validation_pass_rate, prs_created, prs_merged,
          reviews_approved, reviews_changes_requested, review_fixes_applied
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      stmt.run([
        ...
        snapshot.validationPassRate, snapshot.prsCreated, snapshot.prsMerged,
        snapshot.reviewsApproved, snapshot.reviewsChangesRequested, snapshot.reviewFixesApplied,
      ]);
      stmt.free();
    }, { persist: true });
  } catch (err) {
    ...
    await withDb((db) => {
      ...
      for (const row of rows.reverse()) {
        cycleHistory.push({
          ...
          validationPassRate: (row.validation_pass_rate as number) ?? 0,
          prsCreated: (row.prs_created as number) ?? 0,
          prsMerged: (row.prs_merged as number) ?? 0,
          reviewsApproved: (row.reviews_approved as number) ?? 0,
          reviewsChangesRequested: (row.reviews_changes_requested as number) ?? 0,
          reviewFixesApplied: (row.review_fixes_applied as number) ?? 0,
        });
      }
...
export function recordCycle(data: {
  ...
  // PR quality (optional for backward compat)
  prQuality?: {
  ...
  const snapshot: CycleSnapshot = {
    ...
    // PR Quality
    validationPassRate: data.prQuality
      ? (data.prQuality.validationsPassed + data.prQuality.validationsFailed) > 0
        ? data.prQuality.validationsPassed / (data.prQuality.validationsPassed + data.prQuality.validationsFailed)
        : 0
      : 0,
    prsCreated: data.prQuality?.prsCreated ?? 0,
    prsMerged: data.prQuality?.prsMerged ?? 0,
    reviewsApproved: data.prQuality?.reviewsApproved ?? 0,
    reviewsChangesRequested: data.prQuality?.reviewsChangesRequested ?? 0,
    reviewFixesApplied: data.prQuality?.reviewFixesApplied ?? 0,
  };
...
export function getKPIDashboard(): KPIDashboard {
  ...
  // PR Quality aggregates
  const totalPRsCreated = cycleHistory.reduce((s, c) => s + c.prsCreated, 0);
  const totalPRsMerged = cycleHistory.reduce((s, c) => s + c.prsMerged, 0);
  const cyclesWithValidation = cycleHistory.filter((c) => c.prsCreated > 0);
  const prQuality = {
    totalPRsCreated,
    totalPRsMerged,
    mergeRate: totalPRsCreated > 0 ? totalPRsMerged / totalPRsCreated : 0,
    avgValidationPassRate: cyclesWithValidation.length > 0
      ? cyclesWithValidation.reduce((s, c) => s + c.validationPassRate, 0) / cyclesWithValidation.length
      : 0,
    totalReviewsApproved: cycleHistory.reduce((s, c) => s + c.reviewsApproved, 0),
    totalReviewsChangesRequested: cycleHistory.reduce((s, c) => s + c.reviewsChangesRequested, 0),
    totalReviewFixesApplied: cycleHistory.reduce((s, c) => s + c.reviewFixesApplied, 0),
  };
